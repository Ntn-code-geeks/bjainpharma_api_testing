...
class Report_model extends CI_Model {
    ...
    public function sale_report_doctor($userid='',$start='',$end=''){
        ...
        $arr = "pid.id,pid.doc_id, count(DISTINCT(pid.create_date)) total_visits,doc.doc_name as customer,c.city_name as city,pu.name as user,GROUP_CONCAT(msm.sample_name SEPARATOR ',')as sample";
         ...
         if($this->db->affected_rows()){
            $doc_interaction = array();
             ...
             foreach ($doc_sample_info as $k=>$val){
                 ...
                 $arr = "pid.id,`pid`.`doc_id`,count(DISTINCT(pid.create_date)) total_visits,SUM(pid.meeting_sale) as secondary_sale,(SELECT COUNT(pid.meet_or_not_meet) FROM `pharma_interaction_doctor` `pid` WHERE `pid`.`doc_id`='".$val['doc_id']."' AND pid.meet_or_not_meet=1 ) as met,(SELECT COUNT(pid.meet_or_not_meet) FROM `pharma_interaction_doctor` `pid` WHERE `pid`.`doc_id`= '".$val['doc_id']."' AND pid.meet_or_not_meet=0 ) as notmet ";
...
//             pr($result); die;
    ...
    public function sale_report_dealer($userid='',$start='',$end=''){
        ...
        $arr = "pi.id,dl.gd_id as is_cf,`pi`.`d_id`, GROUP_CONCAT(msm.sample_name SEPARATOR ',')as sample, `dl`.`dealer_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user`
...
//             pr($result); die;
     ...
     // sale report for Sub Dealer

    public function sale_report_pharmacy($userid='',$start='',$end=''){

        

        $arr = "pip.id,`pip`.`pharma_id`,`pl`.`company_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user`
...
(SELECT SUM(pid.meeting_sale) as duplicate_secondary FROM pharma_interaction_doctor pid WHERE `pid`.`dealer_id`='".$val['pharma_id']."') as duplicate_secondary";*/
        ...
        $arr = "pip.id,`pip`.`pharma_id`,count(DISTINCT(pip.create_date)) total_visits,SUM(pip.meeting_sale) as secondary_sale,(SELECT COUNT(pip.meet_or_not_meet) FROM `pharma_interaction_pharmacy` `pip` WHERE `pip`.`pharma_id`='".$val['pharma_id']."' AND pip.meet_or_not_meet=1 ) as met,(SELECT COUNT(pip.meet_or_not_meet) FROM `pharma_interaction_pharmacy` `pip` WHERE `pip`.`pharma_id`= '".$val['pharma_id']."' AND pip.meet_or_not_meet=0 ) as notmet ,SUM(pip.duplicate_secondary) as duplicate_secondary,GROUP_CONCAT(pip.duplicate_product) as duplicate_product,GROUP_CONCAT(pip.doctor_id) as  dup_doctor_id";
...
//         echo $this->db->last_query(); die;
    ...
    public function get_tada_report($userid='',$start='',$end=''){
        ...
        $arr = "report.user_id as user_name,report.doi as doi,report.source_city as source_city,report.destination_city as destination_city,report.ta as ta,report.designation_id as designation_id,report.internet_charge as internet_charge,report.distance as distance,report.meet_id as meet_id,report.is_stay as is_stay,report.up_down as up_down,report.created_date as created_date,report.stp_distance as stp_distance,report.stp_ta as stp_ta";
        $this->db->select($arr);
