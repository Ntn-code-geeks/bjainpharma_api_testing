    ...
    public function insert_ta_da($data){
       ...
       if(!is_numeric($data['dealer_view_id']))
       {
        ...
        if($data['up'] && $destination_city==$source_city && $destination_city!=get_user_deatils(logged_user_data())->headquarters_city)
        {
          ...
          $url = "https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=".$source_city."&destinations=".get_user_deatils(logged_user_data())->hq_city_pincode;
          ...
          {
            $cityData=$this->check_city_path($source_city_id,get_user_deatils(logged_user_data())->headquarters_city);
            if($cityData)
            {
