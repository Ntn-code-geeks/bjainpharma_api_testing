...
//         echo $this->db->last_query(); die;
    ...
    public function travel_report_doctor($userid='',$start='',$end='',$page_limit='',$page_start=''){
        ...
        $arr = "pid.telephonic as oncall,pid.id,pid.remark,pid.orignal_sale as order_supply,pid.date_of_supply,`pid`.`meeting_sale` as `secondary_sale`, `pid`.`meet_or_not_meet` as `metnotmet`,pid.create_date as date,pid.doc_id,doc.doc_name as customer,c.city_name as city,pu.name as user";
...
//             pr($result); die;
    ...
    public function travel_report_pharmacy($userid='',$start='',$end='',$page_limit='',$page_start=''){
        ...
        $arr = "pip.telephonic as oncall,pip.id, pip.remark,pip.orignal_sale as order_supply,pip.date_of_supply,`pip`.`pharma_id`, `pl`.`company_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user`,`pip`.`meeting_sale` as `secondary_sale`, `pip`.`meet_or_not_meet` as `metnotmet`,pip.create_date as date ";
...
//         $pharma_name = "pharmacy1";
        ...
        $arr = " `pip`.`pharma_id`, `pl`.`company_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user`,  `pip`.`meeting_sale` as `secondary_sale`, `pip`.`create_date` as `date` ";
...
//         echo $this->db->last_query(); die;
         ...
         if($this->db->affected_rows()){
          ...
          $arr = "`pid`.`doc_id`, `doc`.`doc_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user` ,pid.meeting_sale as secondary_sale, pid.create_date as date ";
...
//         $dealer_id = "1";
        ...
        $arr = " dl.gd_id as is_cf,`pi`.`d_id`, `dl`.`dealer_name` as `customer`, `c`.`city_name` as `city`, `pu`.`name` as `user`, `pi`.`meeting_sale` as `sale`, `pi`.`create_date` as `date` ,pi.meeting_payment as payment, pi.meeting_stock as stock ";
